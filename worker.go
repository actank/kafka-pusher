...
func (this *Worker) delivery(msg *Msg, retry_times int) (success bool, err error) {
	...
	if nil == err {
		...
		if suc == false {
			...
			glog.Errorf("delivery failed,[retry_times:%d][topic:%s][partition:%d][offset:%d][msg:%s][http_code:%d][cost:%v][response_body:%s]", retry_times, msg.Topic, msg.Partition, msg.Offset, rmsg.Data, resp.StatusCode, terpc.Sub(tsrpc) , rbody)
		}
	} else {
		glog.Errorf("delivery failed,[retry_times:%d][topic:%s][partition:%d][offset:%d][msg:%s][error:%s][cost:%v]", retry_times, msg.Topic, msg.Partition, msg.Offset, rmsg.Data, err.Error(), terpc.Sub(tsrpc) )
		suc = false
	}
